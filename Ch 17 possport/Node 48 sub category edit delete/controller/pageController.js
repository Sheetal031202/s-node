...
const logOutFun = (req, res) => {
    // res.clearCookie("adminId") // aa remve karyu chhe ane session destroly karyu che

    req.session.destroy((e) => {
        if (!e) {
            res.redirect("/")
            console.log("session destroy error", e)
...
const changePasswordLogicFun = async (req, res) => {
    try {
        ...
        // 6
        const data = res.locals.currentAdminData   
        ...
        if (req.body.password != data.password) {
            console.log(" pass not match with database")
        ...
        const passwordChange = await adminModel.findByIdAndUpdate(data._id,
             { password: req.body.newPassword }, { new: true })

        if (passwordChange) {
            console.log("change password")
            // 6
            return req.session.destroy((e) => {   
                console.log("Session clear in change password")
                return res.redirect("/")      
            })
        } else {
...
module.exports = {
    ...
    otpVerifyPageShowFun, otpVerifyFun, adminLoginCheckedFun,
    logOutFun, changePasswordFun, forgetPasswordVerifyEmailFun, changePasswordLogicFun,
    profilePageShowFun,
